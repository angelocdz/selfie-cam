...
// Create unattached canvas elements to serve as "layers" of the meme
...
// When either layer has changed, we'll call this function to redraw the meme with the layers' new data
function redrawMeme() {
...
function setupAddText() {
  ...
  saveTextBtn.addEventListener("click", () => { drawText(textLayer); redrawMeme(); });
...
async function setupTakeSelfie() {
  ...
  savePhotoBtn.addEventListener("click", () => { drawVideo(video, selfieLayer); redrawMeme(); });
}
